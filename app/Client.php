...
class Client
{
     ...
     *
     * @param bool|string $proxyhost 可选，代理服务器地址，默认为 false ,则不使用代理服务器
     * @param bool|string $proxyport 可选，代理服务器端口，默认为 false
     * @param bool|string $proxyusername 可选，代理服务器用户名，默认为 false
     * @param bool|string $proxypassword 可选，代理服务器密码，默认为 false
     * @param int|string $timeout 连接超时时间，默认0，为不超时
     * @param int|string $response_timeout 信息返回超时时间，默认30
     *
     */
...
class Mo
{
    ...
    {
        $this->addSerial = $ret[$this->addSerial];
        $this->addSerialRev = $ret[$this->addSerialRev];
        $this->channelnumber = $ret[$this->channelnumber];
        $this->mobileNumber = $ret[$this->mobileNumber];
        $this->sentTime = $ret[$this->sentTime];
        $this->smsContent = $ret[$this->smsContent];
